...
dcat file for those with ≥ 2 cognitive evals can be written out on about line 668
...
#   --------------------------------- Input Downloaded Data Files  --------------------------------
  ...
  dcat <- read.table("Input Files/DataCatalog_10-01-19.csv", header=TRUE, sep=",", na.strings = "")
	enr <- read.table("Input Files/CohortEnrollment_10-01-19.csv", header=TRUE, sep=",", na.strings = "") # Lava CohortEnrollment report 
  asl1 <- read.table("Input Files/AssessList_10-01-19.csv", header=TRUE, sep=",", na.strings = "")
#   hlth <- read.table("Input Files/uds_health_hx_02-07-19.csv", header=TRUE, sep=",", na.strings = "") # semicolon delimited file

  h1 <- readLines("Input Files/UdsHealthHistory_10-01-19.csv") # csv file from Lava
  h2 <- gsub('(?m),(?=[^"]*"(?:[^"\r\n]*"[^"]*")*[^"\r\n]*$)','',h1, perl=TRUE)
...
# Data files to characterize recruitment source
  ...
  mdemo <- read.table("Input Files/MudsDemographicReport_10-01-19.csv", header=TRUE, 
        sep=",", na.strings = "")
  udemo <- read.table("Input Files/UdsSubjectDemographics_10-01-19.csv", header=TRUE, 
        sep=",", na.strings = "")
...
saveRDS(mcisur,file="Output Files/mci_survival.rds")
write.csv(mcisur,file="Output Files/mci_survival.csv",row.names=FALSE)
...
saveRDS(normsur,file="Output Files/normal_survival.rds")
write.csv(normsur,file="Output Files/normal_survival.csv",row.names=FALSE)
...
table_data_cp[["2006-2021"]] <- getT1Stat("ClinDxMRA2","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("2006-07-01") & dcat$DtDeath < as.Date("2021-07-01"),],hrzl_prop=TRUE)
table_data_cp[["2006-2021 Total"]] <- getT1Stat("ClinDxPres","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("2006-07-01") & dcat$DtDeath < as.Date("2021-07-01"),],hrzl_prop=TRUE)
table_data_cp[["1991-2021 Total"]] <- getT1Stat("ClinDxPres","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("1991-10-01") & dcat$DtDeath < as.Date("2021-07-01"),],hrzl_prop=TRUE)
...
table_data_cp[["2006-2021"]] <- getT1Stat("ClinDxMRA2","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("2006-07-01") & dcat$DtDeath < as.Date("2021-07-01") & dcat$Ethnicity %in% c(4,6),],hrzl_prop=TRUE)
table_data_cp[["2006-2021 Total"]] <- getT1Stat("ClinDxPres","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("2006-07-01") & dcat$DtDeath < as.Date("2021-07-01") & dcat$Ethnicity %in% c(4,6),],hrzl_prop=TRUE)
table_data_cp[["1991-2021 Total"]] <- getT1Stat("ClinDxPres","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("1991-10-01") & dcat$DtDeath < as.Date("2021-07-01") & dcat$Ethnicity %in% c(4,6),],hrzl_prop=TRUE)
