...
dcat file can be written out on line 442
date limits for enrollment reports can be changed at 1206
-->
...
#   --------------------------------- Input Downloaded Data Files  --------------------------------
  	...
  	dcat <- read.table("Input Files/adc_data_catalog_11-02-17.csv", header=TRUE, sep=",", na.strings = "")
	enr <- read.table("Input Files/enrollment_11-02-17.csv", header=TRUE, sep=",", na.strings = "") # Lava CohortEnrollment report 
  asl1 <- read.table("Input Files/assesslist_11-02-17.csv", header=TRUE, sep=",", na.strings = "")
#   hlth <- read.table("Input Files/uds_health_hx_02-02-16.csv", header=TRUE, sep=",", na.strings = "") # semicolon delimited file

  h1 <- readLines("Input Files/uds_health_hx_11-02-17.csv") # csv file from Lava
  h2 <- gsub('(?m),(?=[^"]*"(?:[^"\r\n]*"[^"]*")*[^"\r\n]*$)','',h1, perl=TRUE)
  # http://stackoverflow.com/questions/4476812/regular-expressions-how-to-replace-a-character-within-quotes
  h2 <- gsub("'","",h2, perl=TRUE)
...
# Data files to characterize recruitment source
  ...
  mdemo <- read.table("Input Files/muds_demographic_09-21-17.csv", header=TRUE, 
        sep=",", na.strings = "")
  udemo <- read.table("Input Files/uds_subject_demographics_09-21-17.csv", header=TRUE, 
        sep=",", na.strings = "")
		...
		dcat$RecrSource <- ifelse(!is.na(dcat$SENAS_ID),
		    ...
		    ifelse(dcat$REASEVAL %in% c(2,3),
		        ifelse(dcat$REASON %in% c(1,4),"Community",
		            ifelse(dcat$REASON %in% 2,"Clinic","Not Determined")),
...
dcat$edgrp <- ifelse(is.na(dcat$EDUC),NA,
      ifelse(dcat$EDUC < 12,"0-11",
      ifelse(dcat$EDUC == 12,"12",
      ifelse(dcat$EDUC <= 16,"13-16",
      ifelse(dcat$EDUC > 16,"17+",
      NA)))))
...
#	table(dcat$synd_ia)
	...
	dcat2 <- dcat[,c("ID", "DtLastEval", "GENDER", "ETHNICITY", "EDUC", "age_ia", "RecrSource", "Cohort", "center","synd_ia", "synd_mra","grp","edgrp")]
...
ethc2.surv7 <- survfit(Surv(time7,status7) ~ grp, data = eth[eth$RecrSource %in% "Community",])
plot(ethc2.surv7,col=col_names, xlab="Time", ylab="% Retained",cex.lab=1.3)
legend(legend=leg2,"bottomleft", col= col_names, lty=1, lwd=1, box.col=0)
...
eth.surv <- survfit(Surv(time7NLF,status7) ~ edgrp, data = eth)
plot(eth.surv,col=col_names, xlab="Time", ylab="% Retained",cex.lab=1.3)
...
eth.surv7d <- survfit(Surv(time7d,status7d) ~ edgrp, data = eth)
plot(eth.surv7d,col=col_names, xlab="Time", ylab="% Survival",cex.lab=1.3)
legend(legend=leg4,"bottomleft", col= col_names, lty=1, lwd=1, box.col=0)
...
mci.surv <- survfit(Surv(time7Dem,status7) ~ RecrSource, data = mcisur)
...
mci.surv2 <- survfit(Surv(time7Dem,status7) ~ RecrSource, data = mcisur[mcisur$c2orig==1,])
...
norm.surv <- survfit(Surv(time7MCI,status7n) ~ RecrSource, data = normsur)
...
norm.surv2 <- survfit(Surv(time7MCI,status7n) ~ RecrSource, data = normsur[normsur$c2orig==1,])
