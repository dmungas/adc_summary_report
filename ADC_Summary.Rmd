...
```{r, label="r_prelim", message=FALSE, results="hide", echo=FALSE}
...
#   --------------------------------- Input Downloaded Data FIles  --------------------------------
	...
	dcat <- read.table("./Input Files/adc_data_catalog_03-16-15.csv", header=TRUE, sep=",", na.strings = "")
	enr <- read.table("./Input Files/enrollment_03-16-15.csv", header=TRUE, sep=",", na.strings = "")
  asl1 <- read.table("./Input Files/assesslist_03-16-15.csv", header=TRUE, sep=",", na.strings = "")
...
```{r, label="active_syndr_mra",results="asis",message=FALSE,echo=FALSE}

library("Gmisc")
library("Hmisc")

                  
dcat1 <- dcat[dcat$ADCLC_Status=="Active" & !dcat$ID==999999 & !is.na(dcat$synd_mra) & is.na(dcat$ADNI),]  
...
htmlTable(out_data$output_data, align="rrrr",
          ...
          n.cgroup = n.cgroup,
          rowlabel="", 
          caption=paste("Characteristics of currently active longitudinal cohort. Total N = ",nrow(dcat1)," (excluding ",n_adni," ADNI cases).",sep=""), 
...
```{r,label="active_eth",echo=FALSE, results="asis"}
...
par(new=FALSE)
plot(enrstc24$Total ~ enrstc24$year,xlim = c(2001,cur_yr), ylim = c(0,600), type="l", pch = 39, cex = 1.0, cex.axis=1.2, cex.lab=1.2 ,xlab = "Year", ylab = "Number Active", lty=1, lwd=2.5, col = col_names[1])
par(new=TRUE)
...
par(new=FALSE)
plot(enrstc2t4$Total ~ enrstc2t4$year,xlim = c(2001,cur_yr), ylim = c(0,600), type="l", pch = 39, cex = 1.0, cex.axis=1.2, cex.lab=1.2 ,xlab = "Year", ylab = "Number Active", lty=1, lwd=2.5, col = col_names[1])
par(new=TRUE)
...
ini <- filter(noff, type == "Initial") %>%
  select(site, n_office_vis, mnthyr)  %>%
...
visout <- merge(ini[,3:6],fu[,3:6],by="mnthyr",all=TRUE)
...
htmlTable(visout,
          ...
          rowlabel="Month",
          headings = c("East Bay S", "acramento C","ombined","East Bay S", "acramento C","ombined"),
          ctable = TRUE)
...
plot(mean_time_last ~ year, data=tmlstc, type="n",pch=1, xlim=c(2001,cur_yr), ylim=c(0.6,1.5), xlab="Year", ylab = "Average Time Since Last Evaluation")
...
#### Percent of active ADC longitudinal cohort participants with autopsy consent by ethnicity and year
...
# Get the basic stats
table_data_cp[["1991-2006"]] <- getT1Stat("ClinDxMRA2","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("1991-10-01") & dcat$DtDeath < as.Date("2006-07-01") & dcat$ETHNICITY %in% c(4,6),])
table_data_cp[["1991-2006 Total"]] <- getT1Stat("ClinDxPres","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("1991-10-01") & dcat$DtDeath < as.Date("2006-07-01") & dcat$ETHNICITY %in% c(4,6),])
table_data_cp[["2006-2016"]] <- getT1Stat("ClinDxMRA2","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("2006-07-01") & dcat$DtDeath < as.Date("2016-07-01") & dcat$ETHNICITY %in% c(4,6),])
table_data_cp[["2006-2016 Total"]] <- getT1Stat("ClinDxPres","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("2006-07-01") & dcat$DtDeath < as.Date("2016-07-01") & dcat$ETHNICITY %in% c(4,6),])
table_data_cp[["1991-2016 Total"]] <- getT1Stat("ClinDxPres","PathDx",dcat[!dcat$PathDx %in% c("Other","LBD+CVD") & !is.na(dcat$DtDeath) & dcat$DtDeath >= as.Date("1991-10-01") & dcat$DtDeath < as.Date("2016-07-01") & dcat$ETHNICITY %in% c(4,6),])
